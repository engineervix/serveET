...
def execute_bump_hack(c):
    ...
    2. git add pyproject.toml servidora/__init__.py servidora/main.py
    ...
    if release_type == "none":
        ...
        c.run(
            "git add pyproject.toml servidora/__init__.py servidora/main.py",
            pty=True,
        )
...
def run(c, version=False, help=False):
    """run the program"""

    if version:
        c.run("python servidora/main.py -v", pty=True)
    elif help:
        c.run("python servidora/main.py -h", pty=True)
    else:
        c.run("python servidora/main.py", pty=True)
...
def lint(c, fix=False):
    ...
    if fix:
        ...
        c.run("flake8 tests servidora", pty=True)
