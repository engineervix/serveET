...
This is part of my serveEthiopia project, a project aimed at providing
resources for fellow Ethiopians to help them in their IT career.
"""
...
from servidora.db import db_file, helper
from servidora.pveapi import power_op, proxmox, status, vm
from servidora.res import STRINGS
from servidora.util import CONFIG, human_bytes, util
...
def delete_request(requester_id):
    try:
        conn = helper.create_connection(db_file)
        data = helper.delete_request(conn, requester_id)  # noqa: F841
        conn.close()
    except sqlite3.Error as e:
        print(e)  # print for now
...
def nofify_admin(message):
    ...
    msg = util.string_format(
        STRINGS["STR_PROVISION_REQUEST"],
        requester_info["request_time"],
        requester_info["requester_id"],
        requester_info["requesters_username"],
        "pending",
        requester_info["requesters_name"],
        requester_info["os"],
    )
...
def machine_status(message, data):
    ...
    if power_status == "running":
        ...
        # net usage
        net_in = human_bytes.format(stat["msg"]["netin"], precision=2)
        net_out = human_bytes.format(stat["msg"]["netout"], precision=2)
        # load
...
# Dispatcher
POWR_FUNCS = {
    "Power On": power_on_machine,
    "Power Off": power_off_machine,
